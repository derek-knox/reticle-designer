...
export default class EditControlWidget extends Component {
    ...
    @keydownScoped('shift+up', 'shift+right') onKeyShiftAndUpOrRight(e) { this.onSliderChange({ val: this.props.item.settings.val + (this.props.item.settings.step * 10) }) }
    @keydownScoped('shift+down', 'shift+left') onKeyShiftAndDownOrLeft(e) { this.onSliderChange({ val: this.props.item.settings.val - (this.props.item.settings.step * 10) }) }
